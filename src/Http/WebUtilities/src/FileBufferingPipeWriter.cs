...
{
    // TODO: This should probably wright straight to the pipe until a threshold, to optimize for small payloads.
    // Later switch to a file stream approach, probably keeping a buffer to write in large chunks to disk.
    // so faster small write but larger memory usage for big ones, so next will require to find new thresholds.
    /// <summary>
